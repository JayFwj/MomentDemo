...
@implementation VETDiscoverGrayView

-(instancetype)initWithFrame:(CGRect)frame
{
    if (frame.size.width == 0 && frame.size.height == 0) {
        frame.size.width = SCREEN_WIDTH - kDynamicsNormalPadding * 2 - kDynamicsPortraitNamePadding - kDynamicsPortraitWidthAndHeight;;
...
-(UIButton *)grayBtn
{
    if (!_grayBtn) {
        ...
        _grayBtn.backgroundColor = [UIColor colorWithHexString:@"#F0F0F2"];
        WS(weakSelf);
...
- (void)layoutView
{
    ...
    if (_likeArray.count != 0) {
        ...
        _thumbLabel.top = kDynamicsThumbTopPadding;
    ...
    if (_likeArray.count != 0 && _commentArray.count != 0) {
        ...
        _dividingLine.top = (_thumbLabel.bottom + kDynamicsThumbBottomPadding);
    ...
    if (_commentArray.count != 0) {
        ...
        _commentTable.top = lastView == _dividingLine ? lastView.bottom + kDynamicsThumbCommentPadding : lastView.top + 12;
...
@implementation VETDiscoverTableViewCell
...
-(void)setLayout:(NewDynamicsLayout *)layout
{
    ...
    //头像
    _portrait.left = kDynamicsPortraitLeftPadding;
    ...
    _nameLabel.top = kDynamicsNamePaddingTop;
    ...
    _detailLabel.width = SCREEN_WIDTH - kDynamicsPortraitLeftPadding * 2 - kDynamicsPortraitNamePadding - kDynamicsPortraitWidthAndHeight;
    ...
    if (model.shouldShowMoreButton) {
        ...
        if (model.isOpening) {
            [_moreLessDetailBtn setTitle:[@"Discover.FoldTxt" localized] forState:UIControlStateNormal];
        }else{
            [_moreLessDetailBtn setTitle:[@"Discover.AllTxt" localized] forState:UIControlStateNormal];
        }
...
//    }
    ...
    _spreadBtn.top = lastView.bottom + kDynamicsSpreadDetailPadding;;
    ...
    _dateLabel.top = lastView.bottom + kDynamicsTimeSpreadPadding;
    ...
    //点赞，评论
    _commentBtn.size = CGSizeMake(30, 30);
    _commentBtn.right = (SCREEN_WIDTH - 10);
    _commentBtn.centerY = _dateLabel.centerY;
    
    _likeBtn.size = _commentBtn.size;
    _likeBtn.right = (_commentBtn.left - 12);
    _likeBtn.centerY = _dateLabel.centerY;
    ...
    _dividingLine.left = 0;
    _dividingLine.height = .5;
    _dividingLine.width = SCREEN_WIDTH ;
...
-(UIImageView *)portrait
{
    if(!_portrait){
        ...
        [_portrait addConer:20 withBorderWidth:0 withBorderColor:nil];
        WS(weakSelf);
...
-(UIButton *)moreLessDetailBtn
{
    if (!_moreLessDetailBtn) {
        ...
        _moreLessDetailBtn.titleLabel.font = [UIFont systemFontOfSize:12];
        [_moreLessDetailBtn setTitleColor:kDynamicsHightlightColor forState:UIControlStateNormal];
        _moreLessDetailBtn.hidden = YES;
        WS(weakSelf);
...
-(VETPhotoContainerView *)picContainerView
{
    if (!_picContainerView) {
        _picContainerView = [VETPhotoContainerView new];
        _picContainerView.hidden = YES;
    }
...
-(UIButton *)spreadBtn
{
    if (!_spreadBtn) {
        ...
        [_spreadBtn setTitleColor:kDynamicsHightlightColor forState:UIControlStateNormal];
        WS(weakSelf);
...
-(VETDiscoverGrayView *)grayView
{
    if (!_grayView) {
        _grayView = [VETDiscoverGrayView new];
        _grayView.cell = self;
    }
    return _grayView;
}
-(YYLabel *)dateLabel
{
    if (!_dateLabel) {
        ...
        _dateLabel.textColor = [UIColor colorWithHexString:@"#C1C1C4"];
...
-(UIButton *)deleteBtn
{
    if (!_deleteBtn) {
        ...
        [_deleteBtn setTitle:[@"Discover.Del" localized] forState:UIControlStateNormal];
        ...
        [_deleteBtn setTitleColor:kDynamicsHightlightColor  forState:UIControlStateNormal];
        WS(weakSelf);
...
-(UIButton *)likeBtn
{
    if (!_likeBtn) {
        ...
        [_likeBtn setImage:[UIImage imageNamed:@"friend_ico_fav_nm"] forState:UIControlStateNormal];
        WS(weakSelf);
        [_likeBtn bk_addEventHandler:^(id sender) {
            [weakSelf hasSelectedJRMenuIndex:0];
        } forControlEvents:UIControlEventTouchUpInside];
    }
    return _likeBtn;
}
//friend_ico_fav_hl.png
-(UIButton *)commentBtn
{
    if (!_commentBtn) {
        _commentBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _commentBtn.contentMode = UIViewContentModeScaleAspectFit;
        [_commentBtn setImage:[UIImage imageNamed:@"friend_ico_comment_nm"] forState:UIControlStateNormal];
        WS(weakSelf);
        [_commentBtn bk_addEventHandler:^(id sender) {
            [weakSelf hasSelectedJRMenuIndex:1];
        } forControlEvents:UIControlEventTouchUpInside];
    }
    return _commentBtn;
}
...
-(UIView *)dividingLine
{
    if (!_dividingLine) {
        _dividingLine = [UIView new];
        _dividingLine.backgroundColor = [UIColor colorWithHexString:@"#edeeee"];
    }
    return _dividingLine;
}
