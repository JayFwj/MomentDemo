...
- (void)resetLayout
{
    ...
    if (_model.spreadparams.count != 0 || _model.companyparams != 0) {//显示推广
        _height += kDynamicsSpreadDetailPadding;
        _height += kDynamicsSpreadButtonHeight;
    }
    
    _height += kDynamicsTimeSpreadPadding;
    ...
    if (_model.likeArr.count != 0 || _model.commentArr.count != 0) {//底部间隙
        _height += (kDynamicsPortraitNamePadding + kDynamicsBottomPadding) ;
    }else{
        _height += kDynamicsBottomPadding; 
    }
}
...
- (void)layoutDetail
{
    ...
    [detector enumerateMatchesInString:_model.dsp
                            ...
                            usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                                ...
                                if (result.URL) {
                                    ...
                                    [text setColor:kDynamicsHightlightColor range:result.range];
    ...
    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake( SCREEN_WIDTH - kDynamicsPortraitLeftPadding * 2 - kDynamicsPortraitNamePadding - kDynamicsPortraitWidthAndHeight, _model.isOpening ? CGFLOAT_MAX : 16 * lineCount + kDynamicsLineSpacing * (lineCount - 1))];
...
- (void)layoutPicture
{
    ...
    self.photoContainerSize = [VETPhotoContainerView getContainerSizeWithPicPathStringsArray:_model.photocollections];
}
...
- (void)layoutThumb
{
    ...
    for (int i = 0; i < _model.likeArr.count; i++) {
        ...
        [nick setColor:[UIColor colorWithHexString:@"#3E7EB2"] range:nick.rangeOfAll];
    ...
    UIImage *iconImage = [UIImage imageNamed:@"friend_ico_fav_hl"];
    ...
    YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(SCREEN_WIDTH - kDynamicsNormalPadding - kDynamicsPortraitWidthAndHeight - kDynamicsPortraitNamePadding - kDynamicsNameDetailPadding*2 - kDynamicsNormalPadding,CGFLOAT_MAX)];
    ...
    _thumbHeight += kDynamicsThumbBottomPadding;//点赞文字下边距
    
    _thumbCommentHeight += _thumbHeight;
}
- (void)layoutComment
{
    _thumbCommentHeight = _model.likeArr.count == 0 ?  0 : (_thumbCommentHeight + kDynamicsThumbCommentPadding);
    ...
    for (int i = 0; i < _model.commentArr.count; i++) {
        ...
        YYTextHighlight * highLight = [YYTextHighlight new];
        [nick setColor:kDynamicsHightlightColor range:nick.rangeOfAll];
        WS(weakSelf);
        highLight.tapAction = ^(UIView * _Nonnull containerView, NSAttributedString * _Nonnull text, NSRange range, CGRect rect) {
            if (weakSelf.clickUserBlock) {
                weakSelf.clickUserBlock(model.userid);
        ...
        if (model.tonick.length) {
            ...
            [tonick setColor:kDynamicsHightlightColor range:tonick.rangeOfAll];
            ...
            [hfText setColor:kDynamicsNormalColor];
        ...
        [message setColor:kDynamicsNormalColor];
        ...
        YYTextContainer * container = [YYTextContainer containerWithSize:CGSizeMake(SCREEN_WIDTH - kDynamicsPortraitLeftPadding - kDynamicsPortraitWidthAndHeight - kDynamicsPortraitNamePadding - kDynamicsNameDetailPadding*2 - kDynamicsCommentRightPadding,CGFLOAT_MAX)];
        
        YYTextLayout * layout = [YYTextLayout layoutWithContainer:container text:text];
//        _commentHeight += kDynamicsGrayPicPadding;//评论文字上边距
        _commentHeight += layout.textBoundingSize.height;//评论文字高度
        _commentHeight += kDynamicsCommentBottomPadding;//评论文字下边距
