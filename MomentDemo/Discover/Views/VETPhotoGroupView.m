...
- (id)copyWithZone:(NSZone *)zone {
    VETPhotoGroupItem *item = [self.class new];
    return item;
}
@end
...
@property (nonatomic, strong) VETPhotoGroupItem *item;
...
- (void)setItem:(VETPhotoGroupItem *)item {
...
@interface VETPhotoGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation VETPhotoGroupView
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        if (fromView == ((VETPhotoGroupItem *)self.groupItems[i]).thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    VETPhotoGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    VETPhotoGroupItem *item = _groupItems[currentPage];
