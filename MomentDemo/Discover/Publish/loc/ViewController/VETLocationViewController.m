...
@interface VETLocationViewController () <MAMapViewDelegate,MapPoiTableViewDelegate,AMapSearchDelegate,UISearchBarDelegate,SearchResultTableVCDelegate>

@end

@implementation VETLocationViewController
{
    MAMapView *_mapView;
    // 地图中心点的标记
...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem barButtonItemWithTitle:[@"Cancle" localized] titleColor:[UIColor colorWithHexString:@"#000000"] target:self action:@selector(cancelBtnClick:)];
    self.navigationItem.rightBarButtonItem =
    [UIBarButtonItem barButtonItemWithTitle:[@"Done" localized] titleColor:[UIColor colorWithHexString:@"#25CFFF"] target:self action:@selector(doneBtnClick:)];
...
//    [self.navigationController pushViewController:locationDetailVC animated:YES];
    [self.delegate didReceivedSelectedLoc:_tableView.selectedPoi];
    [self dismissViewControllerAnimated:YES completion:nil];
}
...
- (void)setMapCenterWithPOI:(AMapPOI *)point isLocateImageShouldChange:(BOOL)isLocateImageShouldChange
{
    ...
    [self setSendButtonEnabledAfterLoadFinished];
}
...
- (void)initCenterMarker
{
    ...
    _centerMaker.center = CGPointMake(self.view.frame.size.width / 2, (CGRectGetHeight(_mapView.bounds) -  _centerMaker.frame.size.height - TopNavHeight) * 0.5);
...
- (void)initTableView
{
    ...
    [_tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.view).offset(self.view.safeAreaInsets.bottom);
    }];
}
...
- (void)initSearch
{
    ...
    _searchController = [MySearchController instance:_searchResultTableVC withSearchDelegate:nil withSearchResultController:_searchResultTableVC]; 
    ...
    switch (SearchBarStyle) {
        ...
        case 1:  // 点击搜索按钮显示SearchBar
            self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[@"Search" localized] style:UIBarButtonItemStylePlain target:self action:@selector(searchAction)];
...
-(void)showAllTextDialog:(NSString *)str
{
    ...
    _HUD.offset = CGPointMake(_HUD.offset.x, 100.0f);
    //    _HUD.xOffset = 100.0f;
